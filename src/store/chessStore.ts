    ...
    (set, get) => ({
      ...
      triggerComputerMove: (
        ...
        if (engineVersion === "v2") {
          ...
          void getStockfish()
            .getBestMove(game.fen(), { skillLevel, movetime: 500 })
            .then((bestMove) => {
              applyEngineMove(bestMove ?? "", id);
            });
        } else {
          const depth =
...
function applyEngineMove(bestMove: string, id: number) {
  ...
  if (!bestMove || game.isGameOver()) return;

  // UCI moves are like "e2e4" / "e7e8q" — chess.js needs from/to/promotion.
  ...
  try {
    ...
    return; // illegal / desynced
  }

  chessStore.setState({ lastMove: { from, to } });
  syncState();
...
function handleWorkerMessage(e: MessageEvent<{ bestMove: string; id: number }>) {
  applyEngineMove(e.data.bestMove, e.data.id);
}
