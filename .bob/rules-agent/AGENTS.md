...
- **Game logic stays in the store** — `useChessStore` (Zustand) owns all Chess state and worker communication. Components must remain presentational (no direct `Chess` instantiation except `Board.tsx` which creates a read-only instance from the passed FEN to render pieces).
