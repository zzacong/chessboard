...
- **TanStack Router** — routing is file-based under `src/routes/`. `src/routeTree.gen.ts` is auto-generated by the `@tanstack/router-plugin` Vite plugin; it should not be edited manually. There is no `App.tsx`; `main.tsx` bootstraps the router directly.
- **`getChessState()`** — exported from `src/store/chessStore.ts` for reading Zustand state outside React (e.g. in `beforeLoad` route guards). Inside React components, use the `useChessStore(selector)` hook instead.
- **Path aliases** — `@/` maps to `src/`. Component and store imports in all source files use this alias (e.g. `@/components/Board`, `@/store/chessStore`).
- **Worker location** — the chess engine worker is at `src/lib/engine/chessWorker.ts` (not `src/engine/`).
